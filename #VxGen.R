...
# Vaccine type is an input: Vx1 = infant, Vx2 = adult, Vx3 = Combi
...
dV1<-matrix(0,steps,Mnage); dV2<-matrix(0,steps,Mnage); dV3<-matrix(0,steps,Mnage);
# Generate matrix of entering vaccine strata
thetaV1<-matrix(0,steps,Mnage);
thetaV2<-matrix(0,steps,Mnage); thetaV2a<-matrix(0,steps,Mnage); thetaV2m<-matrix(0,steps,Mnage);
...
if (yrintro < yearend){
  ...
  for (i in 1:steps){
    ...
    # If any step is the start of the year then vaccinate 10yos
    if (any(startyr==i)){
      thetaV2a[i,10]<-adultcov # Because of this at the start of the year theta[i,j]*X[i-1,j-1]. As the j-1 have just hit 10yo and are immediately vaccinated
    }
    # If a masscampyear (start of then) then get all adults.
    if (any(masscampyr==i)){
      thetaV2m[i,11:Mnage]<-mcampcov 
    }
  }
  ...
  if (D < 100){ ## D <- 100 if lifelong protection. Then none leave vaccine category. 
    for (i in 1:steps){
      ...
      # Remove from the category those that were vaccinated at age 10. 
...
# For checking output
assign('thetaV1',thetaV1,envir=.GlobalEnv);assign('thetaV2a',thetaV2a,envir=.GlobalEnv);assign('thetaV2m',thetaV2m,envir=.GlobalEnv);