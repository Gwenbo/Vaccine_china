...
manufit<-c()
manufit<-read.csv(paste("paraout_",cntry,".csv",sep=''))[-1]
set<-seq(1,1000,1)
manufit<-cbind(set,manufit)
manufit<-as.data.frame(manufit)
randparam<-as.data.frame(randparam)
randparam[1,]<-manufit[1,]
...
setwd(clusteroutput)
write.table(xout,paste("xout_1",".csv",sep=""),sep=",",row.names=FALSE)
