...
plot(seq(1,8), gaomodel[,1], xaxt = "n", main="China 2013 LTBI estimates: Comparison of data and model", xlab="Age group(years)", ylab="LTBI Prevalence (%)",ylim=c(0,80),type='l', col=4)
axis(1, at=1:8, labels=ages)
points(seq(1,8), gaomodel[,2], type='l', col=3)
points(seq(1,8), gaomodel[,3], type='l', col=2)
legend("topleft", c("Model", "TST", "QFN"), lty=1,col=c(2,3,4), border="white")
